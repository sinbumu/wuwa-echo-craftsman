...
- **메인 설정:** 목표 레벨, 반복 횟수, 옵티마이즈 시행 횟수만 기본 노출. 시작 전 딜레이/클릭 대기/음파통 사용 슬롯 수/폐기 에코 재료 사용 여부 등은 상세 설정 Expander에 숨김.
- **부옵션 설정:** 별도 창에서 잠금을 위한 최소 유효 부옵션 개수, 유효 부옵션 목록, 필수 부옵션 목록, 각 부옵션 최소 수치를 설정. 필수 체크 시 사용도 자동 체크되며, 필수 조건이 모두 만족되어야 잠금 가능.
...
2. 초기 설정 관리 창은 21개 캘리브레이션 항목의 현재 저장 상태(미설정/저장됨), ROI 좌표, asset 파일 경로를 표로 보여준다.
...
7. **수집 타겟 (총 21종):**
   ...
   - **[2/4 에코 강화 기본 화면]:** `roi_expected_level`(강화 후 예상 레벨 OCR), `roi_slot_plus`, `roi_enhance_confirm`, `roi_discard_material_confirm`, `roi_enhance_complete_close`, `roi_optimize_tab`
...
- **ENHANCE:** +0 에코 전제이므로 먼저 `roi_slot_plus` 중앙 클릭 -> 폐기 에코 사용 설정이 켜져 있으면 `roi_material`에서 `icon_discard` 후보 전체 탐색 -> 폐기 에코는 하나씩 클릭하고 매번 `roi_expected_level` OCR 판독, 목표 도달 시 남은 폐기 에코는 클릭하지 않음 -> 폐기 에코를 사용했지만 목표 미달이면 설정된 음파통 영역을 하나씩 클릭하고 매번 OCR 판독 -> 폐기 에코가 없거나 사용 설정이 꺼져 있으면 목표 레벨별 추천 횟수만큼 설정된 음파통 영역을 빠르게 클릭한 뒤 OCR 판독. 목표 레벨 이상이면 `ESC` 1회로 재료 선택장을 닫고 `roi_enhance_confirm` 중앙 클릭 -> 폐기 에코를 재료로 사용했다면 `roi_discard_material_confirm` 중앙 클릭 -> 잠시 대기 -> 강화 완료 오버레이를 `roi_enhance_complete_close` 중앙 클릭으로 닫고 OPTIMIZE 이동. 재료 클릭 5회 이상에도 예상 레벨이 증가하지 않으면 오류 기록 후 정지.
