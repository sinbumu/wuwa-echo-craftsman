...
자동화는 `폐기 에코 강화재료로 사용`이 켜져 있으면 먼저 `roi_material` 안에서 `icon_discard.png` 후보를 모두 이미지 매칭한다. 폐기 에코가 여러 개 보이면 하나씩 클릭하고, 클릭할 때마다 `roi_expected_level`을 OCR로 읽어 목표 도달 여부를 확인한다. 폐기 에코를 사용했지만 목표 예상 레벨에 도달하지 못한 경우에는 음파통도 한 번에 빠르게 누르지 않고, 클릭할 때마다 예상 레벨을 확인한다. 폐기 에코 사용이 꺼져 있거나 폐기 에코가 없으면 기존처럼 상세 설정의 `사용할 음파통 영역 수`만큼 `roi_exp_material_1`부터 순서대로 목표 레벨별 추천 횟수를 빠르게 사용한다.
...
3. 폐기 에코 사용 설정이 켜져 있으면 `roi_material` 안에서 `icon_discard.png` 후보를 모두 찾는다.
4. 폐기 에코 후보가 여러 개 있으면 하나씩 순서대로 클릭하고, 매 클릭 뒤 `roi_expected_level`을 OCR로 읽는다.
5. 폐기 에코 선택 중 예상 레벨이 목표 이상이 되면 남은 폐기 에코는 더 클릭하지 않는다.
6. 폐기 에코를 하나 이상 사용했는데 예상 레벨이 아직 목표보다 낮으면 `roi_exp_material_1`~`roi_exp_material_4` 중 설정된 음파통 영역을 한 번씩 클릭하고 매번 OCR로 예상 레벨을 확인한다.
7. 폐기 에코 사용이 꺼져 있거나 폐기 에코가 없으면 설정된 음파통 영역을 목표 레벨별 추천 횟수만큼 빠르게 클릭한다.
   - 10레벨 목표: 4회
   - 15레벨 목표: 8회
   - 20레벨 목표: 16회
   - 25레벨 목표: 29회
8. 음파통 빠른 투입 뒤에는 다시 `roi_expected_level`을 OCR로 읽는다.
9. 예상 레벨이 목표 레벨 이상이면:
   ...
   - 옵티마이즈 단계로 이동한다.
10. 예상 레벨이 목표 레벨보다 낮으면 다시 재료 투입으로 돌아간다.
11. 재료 클릭을 5회 이상 했는데도 예상 레벨이 증가하지 않으면, 재료 부족 또는 클릭 실패로 보고 오류를 기록한 뒤 중단한다.
12. 강화 시도 반복 제한을 넘거나 필수 재료 영역이 설정되지 않았으면 오류로 중단한다.
