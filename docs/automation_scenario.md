...
자동화는 먼저 `roi_material` 안에서 `icon_discard.png`를 이미지 매칭한다. 폐기 에코가 보이면 해당 아이콘 위치를 클릭한다. 폐기 에코가 없으면 상세 설정의 `사용할 음파통 영역 수`만큼 `roi_exp_material_1`부터 순서대로 사용한다. 예를 들어 값이 1이면 `roi_exp_material_1`만 반복 사용한다.
...
4. 현재 횟수가 목표 시행 횟수보다 작으면 `roi_optimize_plus` 중앙을 클릭한다.
5. 현재 횟수가 목표 시행 횟수보다 크면 `roi_optimize_minus` 중앙을 클릭한다.
6. `roi_optimize_count`가 목표 시행 횟수가 될 때까지 3~5번을 반복한다.
7. 일정 횟수 안에 목표 시행 횟수를 맞추지 못하면 오류를 기록하고 중단한다.
8. 목표 시행 횟수가 확인되면 `roi_optimize_confirm` 중앙을 클릭한다.
9. 잠시 대기한다.
10. `roi_optimize_complete_close` 중앙을 클릭해 옵티마이즈 완료 오버레이를 닫는다.
11. 다시 `roi_substat`을 OCR한다.
12. 이전 OCR 결과와 달라졌으면 옵티마이즈가 완료된 것으로 보고 판정 단계로 이동한다.
...
6. 유효 부옵션 개수가 목표 개수 이상이면 `C`를 입력해 잠금 처리한다.
7. 조건 미달이면 `Z`를 입력해 폐기 처리한다.
8. OCR 원문, 판정 결과, 유효 개수, 시간 정보를 SQLite 히스토리에 저장한다.

## 6.1. 부옵션 설정

`부옵션 설정` 창은 옵티마이즈 후 잠금/폐기 판정에 필요한 설정을 모은다.

- `잠금을 위한 최소 유효 부옵션 개수`: 조건을 만족해야 하는 부옵션 개수다. 0~5 사이로 보정된다.
- `유효 부옵션 목록`: 13종 부옵션 중 사용할 항목을 체크한다.
